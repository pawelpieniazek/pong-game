...
let usernameLeft = localStorage.getItem('usernameleft');
let usernameRight = localStorage.getItem('usernameright');
let winner;
...
let resetBall = function(){
    ball.speedY = 1;
    ball.speedX = (Math.abs(ball.speedX)/ball.speedX)*-7;
}

//move players

let movePlayerLeft = function(event) {
...
let stopPlayerLeft = function(event) {
...
window.addEventListener("keydown", movePlayerLeft);
window.addEventListener("keyup", stopPlayerLeft);
...
const update = function() {
    ...
    if(scoreLeft === 5) {
        gameLive = false;
        winner = usernameLeft;
        $('#winModalLabel').text('Congratulations, '+ winner + "!")
        $('#win').click();

    } else if (scoreRight === 5) {
        gameLive = false;
        winner = usernameRight;
        $('#winModalLabel').text('Congratulations, '+ winner + "!")
        $('#win').click();
    };
...
const draw = function() {
    ...
    ctx.fillText(usernameLeft, GAME_WIDTH/2 - 40, GAME_HEIGHT - 10);
    ctx.fillText(usernameRight, GAME_WIDTH/2 + 20, GAME_HEIGHT - 10);
